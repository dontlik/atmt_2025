...
class MultiHeadedAttention(nn.Module):
    def __init__(self, n_heads: int, dim_embed: int, dropout: float = 0.0, lora_rank: int = 4):
        ...
        #lora 
        self.lora_q_A = nn.Linear(dim_embed, lora_rank, bias=False)
        self.lora_q_B = nn.Linear(lora_rank, dim_embed, bias=False)
        self.lora_v_A = nn.Linear(dim_embed, lora_rank, bias=False)
        self.lora_v_B = nn.Linear(lora_rank, dim_embed, bias=False)

    def forward(self, x_query, x_key, x_value, mask=None):
        nbatch = x_query.size(0) # get batch size
        seq_len = x_query.size(1)
